#!/usr/bin/env Rscript
#
# relationship level granger causality and ARIMAX analysis
...
OUT  <- file.path(E, "SamelagGrangerArimax", "relationship")
if (!dir.exists(OUT)) dir.create(OUT, recursive = TRUE)
DOMS    <- c("care", "hierarchy", "transaction", "mating")
CORPORA <- c("US", "Chinese")
RELS    <- c("parent_child", "close_friends", "romantic_partners", "boss_employee",
             "teacher_student", "doctor_patient", "coworkers", "neighbors")
...
# indi/coll/tight/loose/gdp come from chen et al 2024. US reads its score and
# covariates together from FullCorpusTimeSeriesCovariates/, built by 04_cultural_covariates/.
load_cell <- function(corpus, rel, dom) {
  if (corpus == "Chinese") {
    raw_path <- file.path(E, "RelationshipFullCorpus", "Chinese", rel, paste0("avg__", dom, ".csv"))
    if (!file.exists(raw_path)) return(NULL)
    raw <- read.csv(raw_path); raw <- raw[order(raw$year), ]
    df <- data.frame(year = raw$year, score = raw$mean, n = raw$n)
    df <- df[!is.na(df$score), ]
    df <- df[df$n >= 5, ]
    df <- df[, c("year", "score")]
    df <- merge(df, CHINA_PREVALENCE_2019, by = "year")
  } else {
    path <- file.path(E, "RelationshipFullCorpusCovariates", corpus, rel, paste0("avg__", dom, ".csv"))
...
granger_for_cell <- function(df, corpus, relationship, domain) {
  rows <- list(); k <- 1
  for (p in c("indi", "coll", "tight", "loose")) {
    ...
    fam <- if (p %in% c("indi","coll")) "indi_coll" else "tight_loose"
    rows[[k]] <- data.frame(corpus, relationship, domain, predictor = p, direction = "culture_to_domain",
      family = fam, n_obs = fwd$n_obs, diff_order = fwd$diff_order, best_lag = fwd$best_lag,
      p_value = fwd$p_value, granger_peak_lag = fwd$peak_lag, granger_peak_z = fwd$peak_z,
      stringsAsFactors = FALSE); k <- k + 1
    rows[[k]] <- data.frame(corpus, relationship, domain, predictor = p, direction = "domain_to_culture",
      family = fam, n_obs = rev$n_obs, diff_order = rev$diff_order, best_lag = rev$best_lag,
...
perform_arimax <- function(y, xreg_culture, gdp_z = NULL) {
  ...
  fit <- tryCatch(
    ...
    error = function(e) e)
  if (inherits(fit, "error")) return(list(order = NA, wald_p = NA, lb_p = NA, error = conditionMessage(fit), lag_coefs = NULL))
  ...
  Vsub <- V[ok_names, ok_names, drop = FALSE]; bsub <- b[ok_names]
  ...
  # Per-lag coefficient/SE/z/p -- descriptive (see 01_run_full_corpus_samelag.R's
  # matching comment: the joint Wald test above has no single sign, but each
  # individual lag coefficient does and is individually z-testable).
  se_all <- sqrt(diag(V))
  se <- se_all[ok_names]
  z_vals <- bsub / se
  p_vals <- 2 * pnorm(abs(z_vals), lower.tail = FALSE)

  # Standardized coefficient -- see the matching comment in
  # 01_run_full_corpus_samelag.R's perform_arimax(): raw coefficients are
  # NOT comparable across direction (culture_to_domain vs domain_to_culture)
  # because the response/predictor roles of the raw-scale "score" series and
  # the z-scored culture series swap by direction. Post-hoc rescale by the
  # ACTUAL differenced/aligned series' SDs (the ones actually fit here).
  sd_y <- sd(y)
...
arimax_for_row <- function(df, row) {
  ...
  summary_row <- data.frame(row[c("corpus","relationship","domain","predictor","direction","family")], n_lags = n_lags, diff_order = d,
  ...
  if (!is.null(basic$lag_coefs)) {
    lag_rows <- cbind(row[rep(1, nrow(basic$lag_coefs)), c("corpus","relationship","domain","predictor","direction")],
                       model = "basic", basic$lag_coefs, row.names = NULL)
  }
  if (!is.null(adj$lag_coefs)) {
    adj_rows <- cbind(row[rep(1, nrow(adj$lag_coefs)), c("corpus","relationship","domain","predictor","direction")],
...
for (corpus in CORPORA) for (rel in RELS) for (dom in DOMS) {
  df <- load_cell(corpus, rel, dom)
  if (is.null(df)) { cat("MISSING:", corpus, rel, dom, "\n"); next }
  cat(sprintf("[%s] %s / %s / %s\n", format(Sys.time(), "%H:%M:%S"), corpus, rel, dom))
  cell_dfs[[paste(corpus, rel, dom)]] <- df
  all_granger[[i]] <- granger_for_cell(df, corpus, rel, dom)
  i <- i + 1
}
granger_all <- do.call(rbind, all_granger)

granger_all$p_fdr <- NA_real_
for (co in CORPORA) for (rel in RELS) for (dom in DOMS) for (dir in DIRECTIONS) for (fam in FDR_FAMILIES) {
  idx <- which(granger_all$corpus == co & granger_all$relationship == rel & granger_all$domain == dom &
...
for (i in seq_len(nrow(sig))) {
  key <- paste(sig$corpus[i], sig$relationship[i], sig$domain[i])
  r <- arimax_for_row(cell_dfs[[key]], sig[i, ])
  if (!is.null(r)) {
...
empty_arimax <- function() data.frame(corpus=character(), relationship=character(), domain=character(),
  predictor=character(), direction=character(), family=character(), n_lags=integer(), diff_order=integer(),
...
empty_lag_coefs <- function() data.frame(corpus=character(), relationship=character(), domain=character(),
  predictor=character(), direction=character(), model=character(), lag=integer(), coefficient=numeric(),
  se=numeric(), z=numeric(), p=numeric(), coefficient_std=numeric(), se_std=numeric(), stringsAsFactors=FALSE)

write.csv(granger_all, file.path(OUT, "granger_relationship.csv"), row.names = FALSE)
write.csv(if (!is.null(arimax_all)) arimax_all else empty_arimax(),
          file.path(OUT, "arimax_relationship.csv"), row.names = FALSE)
write.csv(if (!is.null(lag_coefs_all)) lag_coefs_all else empty_lag_coefs(),
          file.path(OUT, "arimax_lag_coefficients_relationship.csv"), row.names = FALSE)

cat("\n=== Granger FDR hits:", sum(granger_all$significant), "/", nrow(granger_all), "===\n")
print(granger_all[granger_all$significant, c("corpus","relationship","domain","predictor","direction","best_lag","p_value","p_fdr")])

if (!is.null(arimax_all)) {
  cat("\n=== ARIMAX follow-up (joint Wald p decides confirmation; peak-lag columns give direction/size) ===\n")
  print(arimax_all[, c("corpus","relationship","domain","predictor","direction","n_lags","basic_wald_p",
